// task 1: Call displayMember for two members: one with email, one without.
function displayMember(id:number , name: string, email?: string): void {
    console.log(`ID: ${id} ,Name: ${name}`);
    if(email){
        console.log(`Email: ${email}`);
    }
}

// task 2: Use calculateFines to sum fines: 5, 10, 2.5.
function calculateFines(...fines:number[]):number{
    let total=0;
    for(let fine of fines){
        total+=fine;
    }
    return total
}

// task 3: Compute a membership fee for $100 with default discount, then with 20%.
function membershipFee(price:number, discountRate:number=0.1):number{
    return price - (price * discountRate);
}

// task 4: Greet visitors “Alice” and “Bob” using both vipGreet and consoleGreet.
function greetVisitor(name:string, greetFn:(name:string)=>void):void{
    greetFn(name);
}

const vipGreet=(name:string):void=>{
    console.log(`Welcome, esteemed VIP ${name}!`);
}

const consoleGreet=(name:string):void=>{
    console.log(`Hello, ${name}. Enjoy your visit.`);
}

// task 5: Compute factorial(5).
function factorial(n:number):number{
    if(n<=1){
        return 1;
    }
    return n * factorial(n-1);
}


// task 6: Generate a text report and a JSON report for an array of sample objects (e.g., { title: "1984" }).
function generateReport(data: object[]): string;
function generateReport(data: object[], format: "json"): string;
function generateReport(data: any[], format?: string): string {
  if (format === "json") {
    return JSON.stringify(data, null, 2);
  }
  return data.map(item => item.toString()).join("\n");
}


displayMember(1, "Alice");
displayMember(2, "Bob", "bob@mail.com");

console.log("Fines:", calculateFines(5, 10, 2.5));

console.log("Fee default:", membershipFee(100));
console.log("Fee 20%:", membershipFee(100, 0.2));

greetVisitor("Alice", vipGreet);
greetVisitor("Bob", consoleGreet);

console.log("Factorial:", factorial(5));

console.log(generateReport([{ title: "1984" }]));
console.log(generateReport([{ title: "1984" }], "json"));